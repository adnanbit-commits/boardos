...
export default function VaultPage() {
  ...
  return (
    <div style={{ padding: '32px 36px', maxWidth: 1020, fontFamily: "'Instrument Sans', system-ui, sans-serif" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Instrument+Sans:wght@400;500;600&display=swap');
        @keyframes spin   { to { transform: rotate(360deg); } }
        @keyframes fadeIn { from { opacity:0; transform:translateY(6px); } to { opacity:1; transform:translateY(0); } }
        .row-anim { animation: fadeIn 0.18s ease; }
        .slot-card:hover { border-color: #5C5750 !important; }
      `}</style>

      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: '#231F1B', margin: 0 }}>Document Vault</h1>
        ...
        <Link href={`/companies/${companyId}`}
          style={{ fontSize: 13, color: '#96908A', textDecoration: 'none', border: '1px solid #E0DAD2', borderRadius: 8, padding: '7px 14px' }}>
          ← Back
        </Link>
      </div>

      {/* Tab bar */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 28, background: '#EBE6DF', borderRadius: 12, padding: 4, width: 'fit-content' }}>
        {(['statutory', 'compliance', 'meetings'] as Tab[]).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{
            background: tab === t ? '#EBE6DF' : 'transparent',
            border: 'none', borderRadius: 9, padding: '8px 18px',
            fontSize: 13, fontWeight: tab === t ? 600 : 400,
            color: tab === t ? '#231F1B' : '#96908A', cursor: 'pointer',
          }}>
    ...
    <div>
      ...
      {loading ? <Spinner /> : (
        <>
            ...
            {STATUTORY_SLOTS.map(slot => {
              ...
              return (
                <div key={slot.docType} className="slot-card row-anim" style={{
                  background: '#FDFCFB', border: `1px solid ${doc ? '#1B3A2A' : '#E0DAD2'}`,
                  borderRadius: 14, padding: '18px 20px', transition: 'border-color 0.15s',
                }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                        {slot.required && <span style={{ fontSize: 9, fontWeight: 700, color: '#8B1A1A', background: 'rgba(139,26,26,0.08)', border: '1px solid rgba(139,26,26,0.22)', padding: '1px 6px', borderRadius: 10, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Required</span>}
                        {doc && <span style={{ fontSize: 9, fontWeight: 700, color: '#166534', background: '#F0FDF4', border: '1px solid #86EFAC', padding: '1px 6px', borderRadius: 10, letterSpacing: '0.06em', textTransform: 'uppercase' }}>✓ Uploaded</span>}
                      </div>
                      <p style={{ fontSize: 14, fontWeight: 700, color: '#231F1B', margin: 0 }}>{slot.label}</p>
                      <p style={{ fontSize: 11, color: '#96908A', margin: '4px 0 0', lineHeight: 1.4 }}>{slot.description}</p>
                    </div>
                  </div>

                  {doc ? (
                    <div style={{ marginTop: 12 }}>
                      <div style={{ background: '#F5F2EE', border: '1px solid #1B3A2A', borderRadius: 8, padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 10 }}>
                        <span style={{ fontSize: 18 }}>📄</span>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <p style={{ fontSize: 12, fontWeight: 600, color: '#231F1B', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{doc.fileName}</p>
                          <p style={{ fontSize: 11, color: '#96908A', margin: '2px 0 0' }}>
                        ...
                        {doc.downloadUrl && (
                          <a href={resolveDownloadUrl(doc.downloadUrl, token)} target="_blank" rel="noopener noreferrer"
                            style={{ fontSize: 11, color: '#8B1A1A', fontWeight: 600, textDecoration: 'none', flexShrink: 0 }}>
                            View ↗
                          </a>
                        )}
                      </div>
                      <button onClick={() => triggerUpload(slot)} disabled={!!uploading}
                        style={{ marginTop: 8, background: 'none', border: 'none', color: '#96908A', fontSize: 11, cursor: 'pointer', padding: 0 }}>
                        Replace file
                      </button>
                    </div>
                  ) : (
                    <button onClick={() => triggerUpload(slot)} disabled={!!uploading}
                      style={{ marginTop: 12, width: '100%', background: isUp ? '#EBE6DF' : '#F5F2EE', border: `1px dashed ${isUp ? '#8B1A1A' : '#E0DAD2'}`, borderRadius: 8, padding: '10px 0', color: isUp ? '#8B1A1A' : '#96908A', fontSize: 12, fontWeight: 600, cursor: isUp ? 'default' : 'pointer' }}>
          ...
          {/* Custom documents */}
          <div style={{ borderTop: '1px solid #E0DAD2', paddingTop: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <p style={{ fontSize: 11, fontWeight: 700, color: '#5C5750', textTransform: 'uppercase', letterSpacing: '0.08em', margin: 0 }}>Other Company Documents</p>
              <button onClick={() => setShowCustom(s => !s)}
                style={{ background: 'none', border: '1px solid #E0DAD2', borderRadius: 8, padding: '6px 14px', color: '#96908A', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                + Add Custom
              </button>
            </div>

            {showCustom && (
              <div style={{ background: '#FDFCFB', border: '1px solid #E0DAD2', borderRadius: 12, padding: '16px 18px', marginBottom: 16, display: 'flex', gap: 10, alignItems: 'flex-end' }}>
                <div style={{ flex: 1 }}>
            ...
            {docs.filter(d => d.docType === 'CUSTOM').map(doc => (
              <div key={doc.id} className="row-anim" style={{ background: '#FDFCFB', border: '1px solid #1B3A2A', borderRadius: 10, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                <span style={{ fontSize: 20 }}>📄</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: '#231F1B', margin: 0 }}>{doc.label}</p>
                  <p style={{ fontSize: 11, color: '#96908A', margin: '2px 0 0' }}>
                ...
                {doc.downloadUrl && (
                  <a href={resolveDownloadUrl(doc.downloadUrl, token)} target="_blank" rel="noopener noreferrer"
                    style={{ fontSize: 12, color: '#8B1A1A', fontWeight: 600, textDecoration: 'none' }}>
                    View ↗
                  </a>
                )}
              </div>
            ...
            {docs.filter(d => d.docType === 'CUSTOM').length === 0 && !showCustom && (
              <p style={{ fontSize: 12, color: '#96908A', textAlign: 'center', padding: '16px 0' }}>No custom documents yet.</p>
            )}
          </div>
        </>
      )}
...
function ComplianceTab({ companyId, token }: { companyId: string; token: string }) {
  ...
  function cellBg(cell: { doc: any; isOverdue: boolean }) {
    if (!cell.doc) return cell.isOverdue ? '#450A0A' : '#FDFCFB';
    if (cell.doc.receivedAt) return '#022C22';
    if (cell.doc.submittedAt) return '#EBE6DF';
    return '#FDFCFB';
  }

  function cellLabel(cell: { doc: any; isOverdue: boolean }) {
    if (!cell.doc) return cell.isOverdue ? <span style={{ color: '#F87171', fontWeight: 700 }}>✕ Overdue</span> : <span style={{ color: '#96908A' }}>— Pending</span>;
    if (cell.doc.receivedAt) return <span style={{ color: '#34D399', fontWeight: 700 }}>✓ Received</span>;
    if (cell.doc.submittedAt) return <span style={{ color: '#1D4ED8', fontWeight: 700 }}>↑ Uploaded</span>;
    return <span style={{ color: '#96908A' }}>— Pending</span>;
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        <span style={{ fontSize: 12, fontWeight: 600, color: '#96908A' }}>Financial Year</span>
        {['2023-24', '2024-25', '2025-26'].map(f => (
          <button key={f} onClick={() => { setFy(f); load(f); }}
            style={{ background: fy === f ? '#EBE6DF' : 'transparent', border: `1px solid ${fy === f ? '#8B1A1A' : '#E0DAD2'}`, borderRadius: 8, padding: '5px 14px', fontSize: 12, fontWeight: fy === f ? 700 : 500, color: fy === f ? '#231F1B' : '#96908A', cursor: 'pointer' }}>
            {f}
          </button>
        ))}
      </div>
        ...
        <>
          ...
          <div style={{ display: 'flex', gap: 16, marginBottom: 16 }}>
            {[
              ...
              { color: '#1D4ED8', label: '↑ Director uploaded' },
              { color: '#F87171', label: '✕ Overdue' },
              { color: '#96908A', label: '— Not submitted' },
            ].map(l => (
              <div key={l.label} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: l.color }} />
                <span style={{ fontSize: 11, color: '#96908A' }}>{l.label}</span>
              </div>
            ))}
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0 }}>
                ...
                <tr>
                  ...
                  {forms.map(f => (
                    <th key={f} style={thStyle}>
                      <div style={{ fontWeight: 700, color: '#231F1B' }}>{COMPLIANCE_FORM_META[f].label}</div>
                      <div style={{ fontSize: 10, color: '#96908A', fontWeight: 400, marginTop: 2 }}>{COMPLIANCE_FORM_META[f].deadlineNote}</div>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {data.matrix.map((row, ri) => (
                  <tr key={row.userId}>
                    <td style={{ padding: '10px 14px', borderBottom: '1px solid #E0DAD2', verticalAlign: 'middle' }}>
                      <p style={{ fontSize: 13, fontWeight: 600, color: '#231F1B', margin: 0 }}>{row.name}</p>
                      <p style={{ fontSize: 11, color: '#96908A', margin: '2px 0 0' }}>{row.role}</p>
                    </td>
                    {row.forms.map(cell => (
                      <td key={cell.formType}
                        onClick={() => { setDrawer({ userId: row.userId, name: row.name, formType: cell.formType }); setNotes(''); }}
                        style={{ padding: '8px 10px', borderBottom: '1px solid #E0DAD2', textAlign: 'center', cursor: 'pointer', background: 'transparent' }}>
                        <div style={{ background: cellBg(cell), border: '1px solid #E0DAD2', borderRadius: 8, padding: '7px 10px', fontSize: 11, transition: 'border-color 0.15s' }}
                          onMouseEnter={e => (e.currentTarget.style.borderColor = '#E0DAD2')}
          ...
          <p style={{ fontSize: 11, color: '#96908A', marginTop: 16, lineHeight: 1.6 }}>
      ...
      {drawer && drawerCell() && (
        <div onClick={() => setDrawer(null)} style={overlayStyle}>
          <div onClick={e => e.stopPropagation()} style={{ background: '#FDFCFB', border: '1px solid #E0DAD2', borderRadius: 18, width: '100%', maxWidth: 460, padding: '28px 28px 24px' }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, color: '#231F1B', margin: '0 0 4px' }}>
              {COMPLIANCE_FORM_META[drawer.formType].label} — {drawer.name}
            </h3>
            <p style={{ fontSize: 12, color: '#96908A', margin: '0 0 20px', lineHeight: 1.5 }}>
              {COMPLIANCE_FORM_META[drawer.formType].description}<br />
              Deadline: {COMPLIANCE_FORM_META[drawer.formType].deadlineNote} · FY {fy}
            </p>

            {drawerCell()?.doc ? (
              <div style={{ background: '#FDFCFB', border: '1px solid #E0DAD2', borderRadius: 10, padding: '12px 16px', marginBottom: 16 }}>
                <p style={{ fontSize: 12, fontWeight: 600, color: '#231F1B', margin: '0 0 6px' }}>📄 {drawerCell()!.doc!.fileName}</p>
                {drawerCell()!.doc!.submittedAt && <p style={{ fontSize: 11, color: '#96908A', margin: '0 0 4px' }}>Uploaded: {new Date(drawerCell()!.doc!.submittedAt!).toLocaleDateString('en-IN')}</p>}
                ...
                {drawerCell()!.doc!.notes && <p style={{ fontSize: 11, color: '#96908A', marginTop: 8, fontStyle: 'italic' }}>Note: {drawerCell()!.doc!.notes}</p>}
              </div>
            ) : (
              <div style={{ background: '#FDFCFB', border: '1px dashed #E0DAD2', borderRadius: 10, padding: '16px', marginBottom: 16, textAlign: 'center' }}>
                <p style={{ fontSize: 12, color: '#96908A', margin: '0 0 12px' }}>No document on file for this form / FY.</p>
                {uploading ? (
                  <div style={{ fontSize: 13, color: '#8B1A1A' }}>Uploading… {uploadPct}%</div>
                ) : (
...
function MeetingPapersTab({ companyId, token }: { companyId: string; token: string }) {
  return (
    <div style={{ textAlign: 'center', padding: '60px 20px', color: '#96908A' }}>
      <div style={{ fontSize: 36, marginBottom: 14 }}>◈</div>
      <p style={{ fontSize: 15, fontWeight: 600, color: '#96908A', marginBottom: 8 }}>Meeting Papers are managed per meeting</p>
      <p style={{ fontSize: 13, lineHeight: 1.6, maxWidth: 440, margin: '0 auto 20px' }}>
        Go to a specific meeting and open the <strong style={{ color: '#231F1B' }}>Documents</strong> tab to upload draft notices,
...
function Spinner() {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 60 }}>
      <div style={{ width: 26, height: 26, border: '2px solid #E0DAD2', borderTop: '2px solid #8B1A1A', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
    </div>
  );
}

const labelStyle: React.CSSProperties = { fontSize: 12, fontWeight: 600, color: '#96908A' };
const inputStyle: React.CSSProperties = { width: '100%', boxSizing: 'border-box', background: '#F5F2EE', border: '1px solid #E0DAD2', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#231F1B', outline: 'none', fontFamily: "'Instrument Sans', system-ui, sans-serif" };
const primaryBtn: React.CSSProperties = { background: '#8B1A1A', color: '#fff', border: 'none', borderRadius: 10, padding: '10px 20px', fontSize: 13, fontWeight: 600, cursor: 'pointer' };
const ghostBtn: React.CSSProperties = { width: '100%', background: '#EBE6DF', color: '#5C5750', border: 'none', borderRadius: 10, padding: '10px 0', fontSize: 13, fontWeight: 600, cursor: 'pointer' };
const overlayStyle: React.CSSProperties = { position: 'fixed', inset: 0, background: 'rgba(35,31,27,0.45)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 };
const thStyle: React.CSSProperties = { padding: '10px 10px', fontSize: 11, fontWeight: 700, color: '#96908A', textTransform: 'uppercase', letterSpacing: '0.06em', borderBottom: '1px solid #E0DAD2', textAlign: 'center', whiteSpace: 'nowrap' };
