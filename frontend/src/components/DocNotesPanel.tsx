...
import { vault as vaultApi, type DocNotesResult } from '@/lib/api';
...
interface Props {
  ...
  onAllNoted?:  () => void; // called when all docs have been noted
}
...
export default function DocNotesPanel({ companyId, meetingId, token, isChairperson, onAllNoted }: Props) {
  ...
  const [noting,     setNoting]     = useState<string | null>(null); // "userId:formType" currently submitting
  const [exception,  setException]  = useState('');
  const [activeCell, setActiveCell] = useState<{ userId: string; name: string; formType: string } | null>(null);
  ...
  return (
    <div>
      ...
      {/* Status banner */}
      <div style={{
        background: allNoted ? '#022C22' : '#1A1F0D',
        border: `1px solid ${allNoted ? '#064E3B' : '#365314'}`,
        borderRadius: 12, padding: '14px 18px', marginBottom: 20,
        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      }}>
        <div>
          <p style={{ fontSize: 13, fontWeight: 700, color: allNoted ? '#34D399' : '#BEF264', margin: '0 0 2px' }}>
            {allNoted ? '✓ All compliance documents noted' : `${data.totalNoted} of ${data.totalRequired} documents noted`}
          </p>
          <p style={{ fontSize: 11, color: allNoted ? '#6B7280' : '#84CC16', margin: 0 }}>
            {allNoted
              ? 'Meeting can now be opened to proceedings.'
              : 'All DIR-8 and MBP-1 declarations must be noted before the meeting can begin.'}
          </p>
        </div>
        <div style={{ textAlign: 'right', flexShrink: 0, marginLeft: 16 }}>
          <div style={{ fontSize: 20, fontWeight: 800, color: allNoted ? '#34D399' : '#BEF264' }}>{progress}%</div>
          <div style={{ width: 80, height: 4, background: '#1a1e26', borderRadius: 2, marginTop: 4 }}>
            <div style={{ width: `${progress}%`, height: '100%', background: allNoted ? '#34D399' : '#84CC16', borderRadius: 2, transition: 'width 0.3s ease' }} />
          </div>
        </div>
      </div>
          ...
          <div key={row.userId} style={{ background: '#13161B', border: '1px solid #232830', borderRadius: 12, overflow: 'hidden' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
              {row.forms.map((cell, ci) => {
                ...
                const isNoting = noting === `${row.userId}:${cell.formType}`;

                return (
                  <div key={cell.formType}
                    style={{ borderRight: ci === 0 ? '1px solid #232830' : 'none', borderTop: '0' }}>
                    <div style={{ padding: '14px 16px' }}>
                      {/* Form label + doc status */}
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                        <span style={{ fontSize: 12, fontWeight: 700, color: '#9CA3AF' }}>{FORM_META[cell.formType].label}</span>
                        {hasDoc
                          ? <span style={{ fontSize: 9, fontWeight: 700, color: '#60A5FA', background: '#1E3A5F', border: '1px solid #1E40AF', padding: '1px 6px', borderRadius: 10, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Doc on file</span>
                          : <span style={{ fontSize: 9, fontWeight: 700, color: '#F87171', background: '#450A0A', border: '1px solid #7F1D1D', padding: '1px 6px', borderRadius: 10, letterSpacing: '0.06em', textTransform: 'uppercase' }}>Not uploaded</span>
                        }
                      </div>
                      <p style={{ fontSize: 11, color: '#4B5563', margin: '0 0 10px', lineHeight: 1.4 }}>{FORM_META[cell.formType].description}</p>

                      {/* Note status or action */}
                      {noted ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          <div style={{ fontSize: 12, fontWeight: 700, color: cell.note!.status === 'NOTED' ? '#34D399' : '#FBBF24' }}>
                        ...
                        <div>
                          {!isActive ? (
                            <button onClick={() => { setActiveCell({ userId: row.userId, name: row.name, formType: cell.formType }); setException(''); }}
                              disabled={isNoting || !data.chairpersonId}
                              style={{ fontSize: 11, fontWeight: 700, color: '#4F7FFF', background: 'rgba(79,127,255,0.08)', border: '1px solid rgba(79,127,255,0.3)', borderRadius: 7, padding: '6px 12px', cursor: 'pointer' }}>
                            ...
                            <div>
                              <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
                                ...
                                <button onClick={() => setActiveCell({ ...activeCell!, _exception: true } as any)}
                              ...
                              {(activeCell as any)?._exception && (
                                <div>
                                  <textarea value={exception} onChange={e => setException(e.target.value)}
                                  ...
                                  <button onClick={() => handleNote(row.userId, cell.formType, 'NOTED_WITH_EXCEPTION')}
