...
export default function ClaimSeatPrompt({ companyId, currentUserName, mcaDirectors, onClaimed, onDismiss }: Props) {
  ...
  return (
    <div style={{ background:'#191D24', border:'1px solid #2A3A6A', borderRadius:16, padding:'24px 28px', marginBottom:24, fontFamily:"'DM Sans',system-ui,sans-serif" }}>
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:12, marginBottom:20 }}>
        <div>
          <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:6 }}>
            <span style={{ fontSize:20 }}>⬢</span>
            <h2 style={{ fontSize:16, fontWeight:700, color:'#F0F2F5', margin:0 }}>Claim your director seat</h2>
          </div>
        ...
        <button onClick={onDismiss} style={{ background:'transparent', border:'none', color:'#4B5563', fontSize:18, cursor:'pointer', lineHeight:1, padding:'0 4px', flexShrink:0 }}>×</button>
      </div>
      {mcaDirectors?.length ? (
        <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
          {mcaDirectors.map(d => {
            ...
            return (
              <div key={d.din} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px', background:isMatch?'#0F2A1A':'#13161B', border:`1px solid ${isMatch?'#166534':'#232830'}`, borderRadius:12 }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap' as const }}>
                    <span style={{ fontSize:14, fontWeight:600, color:'#F0F2F5' }}>{d.name}</span>
                    <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:20, background:'#1A2540', color:'#4F7FFF', border:'1px solid #2A3A6A', textTransform:'uppercase' as const }}>{d.designation}</span>
                  ...
                  <span style={{ fontSize:11, color:'#4B5563', fontFamily:'monospace' }}>DIN: {d.din}{d.appointedOn ? ` · Appointed ${d.appointedOn}` : ''}</span>
                </div>
                <button onClick={() => handleClaim(d.din)} disabled={!!claiming}
                  style={{ padding:'7px 14px', borderRadius:8, fontSize:12, fontWeight:600, cursor:claiming?'wait':'pointer', background:isMatch?'#166534':'transparent', border:`1px solid ${isMatch?'#16a34a':'#374151'}`, color:isMatch?'#FFFFFF':'#9CA3AF', opacity:claiming&&!isBusy?0.5:1, whiteSpace:'nowrap' as const, flexShrink:0 }}>
      ...
      <p style={{ marginTop:14, fontSize:11, color:'#374151', margin:'14px 0 0' }}>Don't see your name? Your name in the app may differ from MCA records. Contact your workspace admin to update your role manually.</p>
    </div>
  );
}
